 ...
 */
@WebServlet(urlPatterns = {"/admin_add_disease"})
public class admin_add_disease extends HttpServlet {
    /**
     * Processes requests for both HTTP <code>GET</code> and <code>POST</code>
     * methods.
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (disease_name.isEmpty() || symptom1.isEmpty()) {
                
                RequestDispatcher req = request.getRequestDispatcher("admin_add_disease_error.jsp");
                ...
                String sql = "INSERT INTO disease (disease_name, symptom1, symptom2, symptom3, care, medicine) VALUES (?, ?, ?, ?, ?, ?)";
                
                PreparedStatement stmt = con.prepareStatement(sql);
                stmt.setString(1, disease_name);
                stmt.setString(2, symptom1);
                stmt.setString(3, symptom2);
                stmt.setString(4, symptom3);
                stmt.setString(5, care);
                stmt.setString(6, medicine);
                stmt.executeUpdate();
                
                RequestDispatcher req = request.getRequestDispatcher("admin_add_disease.jsp");
                req.include(request, response);
